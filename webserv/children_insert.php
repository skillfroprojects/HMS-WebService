...
if (isset($_POST['child_name']) && isset($_POST['child_mother_email']) && isset($_POST['child_father_email'])) {
    ...
    if ($db->isChildExisted($Child_Name,$Child_Mother_Email,$Child_Father_Email)) {
        // user already exists
        $response["response"] = 0;
        $response["message"] = $Child_Name . "already exists";
        ...
        $response["response"] = 1;
        $user = mysql_query("SELECT * from Children_master WHERE (child_name = '$Child_Name' AND child_mother_email = '$Child_Mother_Email') OR (child_name = '$Child_Name' AND child_father_email = '$Child_Father_Email')");
